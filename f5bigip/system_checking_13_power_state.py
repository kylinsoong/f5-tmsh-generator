...
def system_checking(data_str):
    ...
    data = find_content_from_start_end(data_str, "Chassis Power Supply Status", "Chassis Temperature Status")
    ...
    for l in lines:
        ...
        if "Chassis" not in line and "Status" not in line and len(line) > 0:
            raws = line.split(" ")
            power_list = [i.strip() for i in raws if len(trip_prefix(i, None)) > 0]
